...
Public Class Actividades
    ...
    Public Function ObtenerListadoPendientes() As List(Of Actividades)
        ...
        Try
            Dim comando As New SqlCommand()
            comando.Connection = BaseDatos.conexionAgenda
            comando.CommandText = "SELECT A.* FROM Actividades AS A LEFT JOIN ActividadesResueltas AS AR ON A.Id = AR.IdActividad AND A.IdArea = AR.IdArea WHERE A.IdArea=@idArea AND A.IdUsuario=@idUsuario AND AR.IdActividad IS NULL AND AR.IdArea IS NULL AND (A.EsAutorizado = 'FALSE' OR A.EsAutorizado IS NULL)"
